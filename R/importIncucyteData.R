#' importIncucyteData
#'
#' Imports data exported from Incucyte Zoom software.  See the IncucyteZoomExport vignette for more details.
#'
...
importIncucyteData <- function(filepath, metric='pc', plateid=basename(filepath)) {
    ...
    precheck <- read.table(filepath, header=FALSE, sep=';', stringsAsFactors = FALSE)
    start_row <- which (substr(precheck$V1,1,4) == 'Date')
    if (length (start_row) == 0 ) stop ('Please check the input data format is correct')

    data_df <- read.table(filepath,header=TRUE,sep='\t' ,skip=start_row-1, stringsAsFactors = FALSE) %>%
